...
imageDownload = 0""")
    ...
    if imageDownload > 2 or imageDownload < 0:
        imageDownload = 0
    return [onefile, title, imageDownload]
    ...
    if len(link_list) > 1:
        ...
        if option[0] == 2:
            ...
            for l in link_list:
                ...
                print("%-3d " % count, end='')
                status = crawling(l, 1, multiFile=textfile, imagecrawl=imagecrawl, imagetitle=title)
                if status == 404:
            ...
            for l in link_list:
                ...
                print("%4d" % count, end='')
                status = crawling(l, 1, imagecrawl=imagecrawl, imagetitle=title)
                if status == 404:
                    print(link_list[l] + " ...삭제된 페이지입니다")
                else:
                    time.sleep(0.1)
        ...
        for l in link_list:
            status = crawling(l, 1, imagecrawl=imagecrawl)
            if status == 404:
                print(link_list[l] + " ...삭제된 페이지입니다")
...
def single_crawling(URL, option):
    ...
    if URL.isdigit():
        status = crawling(URL, 0, imagecrawl=imagecrawl)
        if status == 404:
            print("삭제된 페이지입니다")
    else:
        status = crawling(URL, 1, imagecrawl=imagecrawl)
        if status == 404:
            print("삭제된 페이지입니다")
...
def crawling(URL, mode, multiFile=None, imagecrawl=False, imagetitle=None):
    text = ''
    while True:
        if mode == 0:
